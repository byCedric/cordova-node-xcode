...
pbxProject.prototype.addToLibrarySearchPaths = function(file) {
    ...
    for (config in configurations) {
        ...
        if (typeof file === 'string') {
            buildSettings['LIBRARY_SEARCH_PATHS'].push(pbxFile.normalizePath(file));
        } else {
...
pbxProject.prototype.hasFile = function(filePath) {
    var files = nonComments(this.pbxFileReferenceSection()),
        normalized = pbxFile.normalizePath(filePath),
        file, id;
    for (id in files) {
        file = files[id];
        if (file.path == normalized || file.path == ('"' + normalized + '"')) {
            return file;
        }
    }
    ...
    var buildConfigurationsList = [
        {
            ...
            buildSettings: {
                ...
                INFOPLIST_FILE: '"' + pbxFile.normalizePath(path.join(targetSubfolder, targetSubfolder + '-Info.plist')) + '"',
...
function pbxFileReferenceObj(file) {
    var fileObject = {
        ...
        name: "\"" + file.basename + "\"",
        path: "\"" + pbxFile.normalizePath(file.path) + "\"",
...
function searchPathForFile(file, proj) {
    var plugins = proj.pbxGroupByName('Plugins'),
        ...
        fileDir = pbxFile.normalizePath(path.dirname(file.path));
